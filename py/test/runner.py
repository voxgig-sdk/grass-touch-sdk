# GrassTouch SDK test runner

from __future__ import annotations
import os
import json

from grasstouch_sdk.utility.voxgig_struct import voxgig_struct as vs


class GrassTouchTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GrassTouchTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GrassTouchTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GrassTouchTestRunner.getenv("GRASS_TOUCH_TEST_LIVE")
        override = GrassTouchTestRunner.getenv("GRASS_TOUCH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GrassTouchTestRunner.getenv(key)
        ...
        explain = GrassTouchTestRunner.getenv("GRASS_TOUCH_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if GrassTouchTestRunner._test_control is not None:
            return GrassTouchTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GrassTouchTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GrassTouchTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GrassTouchTestRunner.load_test_control()
...
def load_env_local():
    GrassTouchTestRunner.load_env_local()


def env_override(m):
    return GrassTouchTestRunner.env_override(m)


def entity_data(v):
    return GrassTouchTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GrassTouchTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GrassTouchTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GrassTouchTestRunner.load_test_control()


def live_client_options():
    return GrassTouchTestRunner.live_client_options()


def live_delay_ms():
    return GrassTouchTestRunner.live_delay_ms()
