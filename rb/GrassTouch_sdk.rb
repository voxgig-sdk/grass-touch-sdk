...
require_relative 'GrassTouch_types'
...
class GrassTouchSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GrassTouchHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GrassTouchHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GrassTouchFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GrassTouchUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GrassTouchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GrassTouchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GrassTouchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = GrassTouchHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GrassTouchError.new(
        "graphql_error", "GrassTouchSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
