...
module GrassTouchUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GrassTouchResponse.new({})
    ...
    if fetch_err
      ...
      response = GrassTouchResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GrassTouchResponse.new(fetched)
    else
