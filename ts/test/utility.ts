...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.GRASSTOUCH_TEST_LIVE ||
    'TRUE' === process.env.GRASSTOUCH_TEST_OVERRIDE
  ) {
  ...
  m.GRASSTOUCH_TEST_EXPLAIN = process.env.GRASSTOUCH_TEST_EXPLAIN || m.GRASSTOUCH_TEST_EXPLAIN
