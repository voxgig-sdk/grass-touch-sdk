...
//     (\`new GrassTouchSDK(...)\` / \`GrassTouchSDK.test(...)\` become
//     \`GrassTouchSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_grass_touch_status":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GrassTouchSDK(...)\` and any
// \`GrassTouchSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GrassTouchSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GrassTouchSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
