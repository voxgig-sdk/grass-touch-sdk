...
boru-driven command-line client **and** interactive REPL for the GrassTouch
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/grass-touch-cli)
make build

# 2. See usage (words, entities, env vars)
./grass-touch-cli --help

# 3. Provide credentials once, via the environment
export GRASS_TOUCH_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./grass-touch-cli load 1 get_grass_touch_status            # {id:1} shorthand
./grass-touch-cli load '{id:1}' get_grass_touch_status       # explicit match map

# 5. Override the API base URL for a single call
GRASS_TOUCH_BASE=https://api.example.com ./grass-touch-cli load 1 get_grass_touch_status

# 6. No arguments -> interactive REPL
./grass-touch-cli
grass-touch> load 1 get_grass_touch_status
grass-touch> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/grass-touch-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/grass-touch-cli load 1 get_grass_touch_status
   ```
...
```sh
./grass-touch-cli load 1 get_grass_touch_status          # scalar shorthand for {id:1}
./grass-touch-cli load '{id:1}' get_grass_touch_status     # explicit match map
```
...
./grass-touch-cli load 1 get_grass_touch_status
```
...
Run with no arguments to open a REPL (prompt `grass-touch>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_grass_touch_status`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/grass-touch-cli`. |
...
(`./grass-touch-cli <expr>`) and interactively (the REPL), and expressions compose the
