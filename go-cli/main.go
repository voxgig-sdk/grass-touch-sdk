// Command GrassTouch-cli is a CLI + REPL for the GrassTouch SDK,
...
const prompt = "grass-touch"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_grass_touch_status"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GRASS_TOUCH_APIKEY (api key), GRASS_TOUCH_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GRASS_TOUCH_APIKEY carries the API key and
	// GRASS_TOUCH_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GRASS_TOUCH_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GRASS_TOUCH_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGrassTouchSDK(opts)
