...
local GrassTouchSDK = {}
GrassTouchSDK.__index = GrassTouchSDK
...
GrassTouchSDK._make_feature = _make_feature


function GrassTouchSDK.new(options)
  local self = setmetatable({}, GrassTouchSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function GrassTouchSDK:options_map()
...
function GrassTouchSDK:get_utility()
  return Utility.copy(self._utility)
end


function GrassTouchSDK:get_root_ctx()
  return self._rootctx
end


function GrassTouchSDK:prepare(fetchargs)
...
function GrassTouchSDK:direct(fetchargs)
...
-- by passing it.
function GrassTouchSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function GrassTouchSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "GrassTouchSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:GetGrassTouchStatus():list() / client:GetGrassTouchStatus():load({ id = ... })
...
function GrassTouchSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = GrassTouchSDK.new(sdkopts)
  sdk.mode = "test"
