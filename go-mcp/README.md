...
[MCP](https://modelcontextprotocol.io) server exposing the GrassTouch SDK as
two agent tools — `grass-touch_list` and `grass-touch_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/grass-touch-mcp)
make build

# 2. Provide credentials via the environment
export GRASS_TOUCH_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user grass-touch \
  -- /absolute/path/to/grass-touch-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./grass-touch-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// grass-touch_list: first page of records
{ "entity": "get_grass_touch_status" }
{ "entity": "get_grass_touch_status", "query": { } }

// grass-touch_load: one record by id
{ "entity": "get_grass_touch_status", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/grass-touch-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user grass-touch \
     -- "$PWD"/dist/*/grass-touch-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `grass-touch_list` and `grass-touch_load` tools now appear
   in new sessions. Ask the agent to *"list get_grass_touch_status using grass-touch"*
   and it calls `grass-touch_list` with `{"entity":"get_grass_touch_status"}`.
...
```sh
export GRASS_TOUCH_APIKEY=sk_live_xxx            # API key
export GRASS_TOUCH_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `grass-touch_list` tool
...
### Call the `grass-touch_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `grass-touch_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `grass-touch_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GRASS_TOUCH_APIKEY` | API key sent with every request. |
| `GRASS_TOUCH_BASE` | Optional override of the API base URL. |
...
```sh
./grass-touch-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"grass-touch_load","arguments":{"entity":"get_grass_touch_status","query":{"id":1}}}}'
```
