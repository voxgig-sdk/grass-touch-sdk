...
{
    private static function utility(): GrassTouchUtility
    {
        return new GrassTouchUtility();
    }
    ...
    private static function ctx(array $over = []): GrassTouchContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GrassTouchContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GrassTouchOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GrassTouchError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GrassTouchOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GrassTouchMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GrassTouchOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GrassTouchMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GrassTouchOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GrassTouchMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GrassTouchSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GrassTouchMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GrassTouchResponse([]),
            'result' => new GrassTouchResult([]),
        ]);
        [, $err] = GrassTouchMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GrassTouchSpec([]),
            'response' => null,
            'result' => new GrassTouchResult([]),
        ]);
        [, $err] = GrassTouchMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GrassTouchSpec(['step' => 's']),
            'response' => new GrassTouchResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GrassTouchResult([]),
        ]);
        [, $err] = GrassTouchMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GrassTouchSpec(['step' => 's']),
            'response' => new GrassTouchResponse(self::resp(200, ['v' => 1])),
            'result' => new GrassTouchResult([]),
        ]);
        [, $err] = GrassTouchMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GrassTouchSpec(['step' => 's']),
            'response' => new GrassTouchResponse(self::resp(200, ['v' => 2])),
            'result' => new GrassTouchResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GrassTouchMakeResponse::call($ctx);
    ...
    {
        $preset = new GrassTouchResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GrassTouchSpec([]),
            'response' => new GrassTouchResponse([]),
            'result' => new GrassTouchResult([]),
        ...
        [$got, $err] = GrassTouchMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GrassTouchResult([])]);
        [, $err] = GrassTouchMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GrassTouchSpec([]), 'result' => null]);
        [, $err] = GrassTouchMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GrassTouchSpec(['step' => 's']),
            'result' => new GrassTouchResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GrassTouchOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GrassTouchMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GrassTouchSpec(['step' => 's']),
            'result' => new GrassTouchResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GrassTouchOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GrassTouchMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GrassTouchResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GrassTouchSpec([]),
            'result' => new GrassTouchResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GrassTouchMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GrassTouchMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GrassTouchError('boom', 'boom');
        $utility->fetcher = function (GrassTouchContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GrassTouchSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GrassTouchMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GrassTouchContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GrassTouchMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GrassTouchResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        GrassTouchMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GrassTouchContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GrassTouchSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GrassTouchMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GrassTouchResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GrassTouchSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GrassTouchMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GrassTouchMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GrassTouchMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GrassTouchResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GrassTouchDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GrassTouchResult(['ok' => false])]);
        $this->expectException(GrassTouchError::class);
        GrassTouchDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GrassTouchResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GrassTouchMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GrassTouchMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GrassTouchBaseFeature();
        $b = new GrassTouchBaseFeature();
        GrassTouchFeatureAdd::call($ctx, $a);
        GrassTouchFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GrassTouchBaseFeature
    {
        $f = new GrassTouchBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GrassTouchFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GrassTouchContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GrassTouchSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GrassTouchPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        GrassTouchPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GrassTouchResponse(['headers' => null]),
            'result' => new GrassTouchResult([]),
        ]);
        GrassTouchResultHeaders::call($ctx);
