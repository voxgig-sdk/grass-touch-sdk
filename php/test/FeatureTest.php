...
class FtCtrl extends GrassTouchControl
{
...
class FtClient
{
    ...
    private GrassTouchUtility $utility;

    public function __construct(GrassTouchUtility $utility, array $options)
    {
    ...
    public function get_utility(): GrassTouchUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GrassTouchContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GrassTouchContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GrassTouchContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GrassTouchConfig::make_feature($name);
    ...
    private static function buildUrl(GrassTouchSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GrassTouchError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GrassTouchResponse($fetched);
        }
    ...
    private function fail(GrassTouchContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GrassTouchContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
