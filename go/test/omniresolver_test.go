...
import (
	...
	omni "github.com/voxgig-sdk/grass-touch-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GrassTouchError); is {
				out := map[string]any{
					"name":    "GrassTouchError",
...
func omniCtx(arg any, client *sdk.GrassTouchSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GrassTouchSDK); is {
			client = live
